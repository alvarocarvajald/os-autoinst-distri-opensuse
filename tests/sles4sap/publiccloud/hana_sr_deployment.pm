...
use qesapdeployment;
use hacluster qw(add_to_known_hosts);
use Data::Dumper;
use Storable;
...
sub qesap_get_variables {
    ...
    $variables{IMAGE_OWNER} = get_var('PUBLIC_CLOUD_IMAGE_OWNER', 'self');
...
sub run {
    ...
    qesap_create_aws_credentials($provider->provider_client->key_id, $provider->provider_client->key_secret)
    ...
    for my $i (@instances) {
        $i->wait_for_ssh();
        add_to_known_hosts($i->public_ip);
    }
    # exit early. not testing ansible ATM
    $self->{instances} = $run_args->{instances} = \@instances;
    $run_args->{my_provider} = $provider;
    return 1;
    my @instances_export;
    qesap_execute(cmd => 'ansible', verbose => 1);
